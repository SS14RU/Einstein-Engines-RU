...
ARRAY_HIGHP vec3 rgb2hsv( ARRAY_HIGHP vec3 c) {
...
ARRAY_HIGHP vec3 hsv2rgb( ARRAY_HIGHP vec3 c) {
 highp vec4 K = vec4 ( 1.0 , 2.0 / 3.0 , 1.0 / 3.0 , 3.0 ) ;
 highp vec3 p = abs ( fract ( c . xxx + K . xyz ) * 6.0 - K . www ) ;
 return c . z * mix ( K . xxx , clamp ( p - K . xxx , 0.0 , 1.0 ) , c . y ) ;

}
ARRAY_HIGHP float rng( ARRAY_HIGHP vec2 co) {
 return fract ( sin ( dot ( co . xy , vec2 ( 12.9898 , 78.233 ) ) ) * 43758.5453 ) ;

}
ARRAY_HIGHP vec3 blend( ARRAY_HIGHP vec3 base,  ARRAY_HIGHP vec3 color,  ARRAY_HIGHP float factor) {
...
void main()
{
 ...
 col = hsv2rgb ( col ) ;
 COLOR = vec4 ( mix ( col , col2 , 0.3 ) , max ( tex . w * ( sin ( TIME * sin ( TIME ) ) ) * 0.25 , 0 ) + 0.45 * tex . w ) ;
